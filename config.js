...
try {
            ...
            if (el.attrib.name === 'android' || el.attrib.name === 'ios') {
                el._children.forEach(function(child) {
                    if (child.tag === 'content') {
                        console.log('Setting ' + el.attrib.name + ' src to ' + src[el.attrib.name]); // eslint-disable-line no-console
    ...
    console.error('ERROR: Could not replace content src in: ' + config, err); // eslint-disable-line no-console
    process.exit(1);
}